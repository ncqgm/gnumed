...
def test():
	...
	ic = resolver.getIdentificationComponent()
...
def __getTestProfiles(idMgr):
	...
	#  ( _ is space)
	a = "123 Black Rd^^Blacktown^Erewon^0001^Erewhere^ ^ "
...
def _test_find_or_register_id(idMgr):
	...
	print "Testing inserting single profile twice"
...
def _test_register_new_ids(idMgr):
	...
	try:
		...
		if sys.exc_info()[0] == PersonIdService.ProfileExists:
			print "PASS: the correct eception ProfileExists was thrown."
		else:
...
def _test_register_these_ids(idMgr):
	...
	try:
		...
		if sys.exc_info()[0] == PersonIdService.IdsExist:
