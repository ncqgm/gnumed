...
def getTraitSelectorSeqFromProfile( profile, specifiedTraits, defaultWeight = 0.2 ):
	...
	return [ PersonIdService.TraitSelector( trait, defaultWeight) for trait in filter( lambda(t): t.name in traitnames and t.value.typecode().kind() == CORBA.tk_string and t.value.value().strip() <> '' , profile ) ]
