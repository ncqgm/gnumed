...
select
	...
	'a'::text
		as soap_cat,
	(select _('Health Issue')) || ' ('
			|| case when c_hi.is_active
				then (select _('active'))
				else (select _('inactive'))
			   end
			|| ', '
			|| case when c_hi.clinically_relevant
				then (select _('clinically relevant'))
				else (select _('clinically not relevant'))
			   end
			|| coalesce((', ' || c_hi.diagnostic_certainty_classification), '')
			|| '): '
			|| c_hi.description
			|| coalesce((E';\n' || (select _('noted at age')) || ': ' || c_hi.age_noted::text || E';\n'), E';\n')
			|| coalesce(((select _('Laterality')) || ': ' || c_hi.laterality || ' / '), '')
			|| case when c_hi.is_confidential
				then (select _('confidential')) || ' / ' else ''
			   end
			|| case when c_hi.is_cause_of_death
				then (select _('cause of death')) else ''
			   end
			|| coalesce((E';\n' || (select _('Summary')) || E':\n' || c_hi.summary), E'')
			|| coalesce ((
					E';\n' || array_to_string (
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_health_issues_journal.sql', '23.0');
