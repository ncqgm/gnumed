...
create view clin.v_reminders_journal as
	...
	select
		...
		NULL::text
			as soap_cat,
		(select _('Due today')) || ' (' || d_vit.l10n_category || ' - ' || d_vit.l10n_type || ')' || E'\n'
			|| coalesce(' ' || d_mi.comment || E'\n', '')
			|| coalesce(' ' || (select _('Will expire:')) || ' ' || to_char(d_mi.expiry_date, 'YYYY-MM-DD') || E'\n', '')
			|| ' ' || (select _('Importance:')) || ' ' || d_mi.importance || E'\n'
			|| coalesce(' ' || (select _('Context:')) || ' ' || array_to_string(d_mi.ufk_context, ',', '?') || E'\n', '')
			|| coalesce(' ' || (select _('Data:')) || ' ' || d_mi.data || E'\n', '')
			|| coalesce(' ' || (select _('Provider:')) || ' ' || d_st.short_alias, '')
		as narrative,
		(
			select c_e.pk
...
UNION
	...
	select
		...
		NULL::text
			as soap_cat,
		(select _('Epires today')) || ' (' || d_vit.l10n_category || ' - ' || d_vit.l10n_type || ')' || E'\n'
			|| coalesce(' ' || d_mi.comment || E'\n', '')
			|| coalesce(' ' || (select _('Was due:')) || ' ' || to_char(d_mi.due_date, 'YYYY-MM-DD') || E'\n', '')
			|| ' ' || (select _('Importance:')) || ' ' || d_mi.importance || E'\n'
			|| coalesce(' ' || (select _('Context:')) || ' ' || array_to_string(d_mi.ufk_context, ',', '?') || E'\n', '')
			|| coalesce(' ' || (select _('Data:')) || ' ' || d_mi.data || E'\n', '')
			|| coalesce(' ' || (select _('Provider:')) || ' ' || d_st.short_alias, '')
...
UNION
	...
	select
		...
		NULL::text
			as soap_cat,
		(select _('Clinical reminder')) || ' (' || d_vit.l10n_category || ' - ' || d_vit.l10n_type || ')' || E'\n'
			|| coalesce(' ' || d_mi.comment || E'\n', '')
			|| coalesce(' ' || (select _('Due:')) || ' ' || to_char(d_mi.due_date, 'YYYY-MM-DD') || E'\n', '')
			|| coalesce(' ' || (select _('Expires:')) || ' ' || to_char(d_mi.expiry_date, 'YYYY-MM-DD') || E'\n', '')
			|| ' ' || (select _('Importance:')) || ' ' || d_mi.importance || E'\n'
			|| coalesce(' ' || (select _('Context:')) || ' ' || array_to_string(d_mi.ufk_context, ',', '?') || E'\n', '')
			|| coalesce(' ' || (select _('Data:')) || ' ' || d_mi.data || E'\n', '')
			|| coalesce(' ' || (select _('Provider:')) || ' ' || d_st.short_alias, '')
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_reminders_journal.sql', '23.0');
