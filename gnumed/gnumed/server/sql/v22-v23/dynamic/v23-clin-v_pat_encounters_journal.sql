...
select
	...
	(select _('Encounter')) || ': '
			...
			|| coalesce(E'\n @ ' || (select _('branch')) || ' "' || d_ou.description || '" ' || (select _('of')) || ' "' || d_o.description || '"', '')
			|| coalesce(E'\n ' || (select _('RFE')) || ': ' || c_enc.reason_for_encounter, '')
			|| coalesce ((
					E'\n' || array_to_string (
			...
			|| coalesce(E'\n ' || (select _('AOE')) || ': ' || c_enc.assessment_of_encounter, '')
			|| coalesce ((
					E'\n' || array_to_string (
						(select array_agg(r_csr.code || ' (' || r_ds.name_short || ' - ' || r_ds.version || ' - ' || r_ds.lang || '): ' || r_csr.term)
						 from
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_pat_encounters_journal.sql', '23.0');
