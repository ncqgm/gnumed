...
select
	...
	c_all.soap_cat
		as soap_cat,
	(select _('Allergy')) || ' (' || _(c_at.value) || '): ' || coalesce(c_all.narrative, '') || E'\n'
		|| (select _('substance')) || ': ' || c_all.substance || '; '
		|| coalesce(((select _('allergene')) || ': ' || c_all.allergene || '; '), '')
		|| coalesce(((select _('generic'))   || ': ' || c_all.generics || '; '), '')
		|| coalesce(((select _('ATC code'))  || ': ' || c_all.atc_code || '; '), '')
		as narrative,
	c_all.fk_encounter
		as pk_encounter,
	c_all.fk_episode
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_pat_allergies_journal.sql', '23.0');
