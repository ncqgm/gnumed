...
select
	inner_export_items.*,
	(inner_export_items.pk_identity IS DISTINCT FROM inner_export_items.pk_identity_raw)
...
from (
	select
		c_ei.pk
			as pk_export_item,
		-- always correct, even if the patient referenced
		-- via .fk_doc_obj was changed under us and thusly
		-- does not correspond with .fk_identity anymore
		CASE
			WHEN c_ei.fk_doc_obj IS NULL THEN
				c_ei.fk_identity
			ELSE (
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_export_items.sql', '23.0');
