...
comment on function gm.create_enc_epi_sanity_check_trigger(_schema_name name, _table_name name, _fk_encounter_col name, _fk_episode_col name) is
	'This function can be run on any table in order to add enccounter <-> episode patient sanity check triggers to that table.';
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-sanity_check_patient_from_encounter_vs_episode.sql', '23.0');
