...
select
	...
	case
		when r_vah.is_active is TRUE then
			(select _('Active hint'))
		else
			(select _('Inactive hint'))
	end
		|| ' #' || c_sh.fk_hint || ' ' || (select _('suppressed by')) || ' ' || c_sh.suppressed_by || E'\n'
		|| coalesce((select _('Title: ')) || r_vah.title || E'\n', '')
		|| coalesce((select _('URL: ')) || r_vah.url || E'\n', '')
		|| coalesce((select _('Source: ')) || r_vah.source || E'\n', '')
		|| coalesce((select _('Rationale: ')) || c_sh.rationale || E'\n', '')
		|| case when c_sh.md5_sum <> r_vah.md5_sum
			then (select _('Hint definition has been modified since suppression. Rationale for suppression may no longer apply.')) || E'\n'
			else ''
		end
		|| coalesce((select _('Hint: ')) || r_vah.hint || E'\n', '')
		|| coalesce((select _('Recommendation: ')) || r_vah.recommendation, '')
		as narrative,
	c_sh.fk_encounter
		as fk_encounter,
	NULL::integer
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v23-clin-v_suppressed_hints_journal.sql', '23.0');
