%------------------------------------------------------------------
% GNUmed default template for most recent lab results
%
...
\usepackage{pbox}							% improved table cells
\usepackage{booktabs}						% improved tables typesetting
\usepackage{xltabular}						% bessere Tabellen (mehrere Seiten, X-Spalten)
% debugging
...
% %% $m[PtLASTFirst==$1<lastname::::1>1$\textsc{$1<lastname::::2-99>1$}, $1<firstname::::>1$]m$
...
% - PDF metadata
\hypersetup{
	pdftitle = {Current Medication List: $m[PtFirstLast]m$, $m[PtDOB]m$},
...
\lohead{{%
	\parbox[t]{\textwidth}{%
		\upshape
		\centering
		{\Large Most recent lab results}\\
		$<today::%d.%B %Y::>$ -- Page \thepage{} of \pageref{LastPage}\\
		\begin{tabular}[t]{ll}%
			Patient & {\large $m[PtLASTFirst]m$}\\
			born    & $m[PtDOB]m$\\
...
$<free_text::Note on lab results (below lab table)::9999>$
