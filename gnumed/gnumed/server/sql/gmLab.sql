...
COMMENT ON TABLE lab_test IS "description of a test as provided by a specific lab, this table can be built either from an ELV (Elektronisches LeistungsVerzeichnis = electronic methods directory) provided by the labs or dynamically during import of lab results, or manually of course";
COMMENT ON COLUMN lab_test.lab_id IS "link to lab record";
COMMENT ON COLUMN lab_test.short_name IS "short name as provided by a specific lab";
COMMENT ON COLUMN lab_test.long_name IS "long name as provided by a specific lab";
COMMENT ON COLUMN lab_test.test_id IS "link to test in our practice";

-- ====================================
CREATE TABLE lab_result (
	pat_id INTEGER REFERENCES identity,
	episode_id INTEGER REFERENCES episode default 0;	-- episode 0 == "unattached"
