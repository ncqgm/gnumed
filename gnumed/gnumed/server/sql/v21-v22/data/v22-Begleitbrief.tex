...
% - reference line
\setkomavar{yourref}{$<free_text::"Ihr Zeichen"::40>$}
\setkomavar{yourmail}{$<free_text::"Ihr Schreiben vom"::20>$}
\setkomavar{myref}{$<free_text::"Unser Zeichen"::40>$}
...
\setkomavar{subject}[]{%
	...
	$<<<if_not_empty::$<<free_text::Betreff (Patientenstammdaten werden hinzugefügt)::120>>$#3#%s\\#3#% no additional Subject by user::>>>$
	$<<<ph_cfg::argumentsdivider//DEFAULT//%% <%(name)s> set to [%(value)s]::>>>$
	$<<ph_cfg::argumentsdivider//#2#//%% <%(name)s> set to [%(value)s]::>>$
...
\setkomavar{signature}{%
	$<<<ph_cfg::argumentsdivider//#3#//%% <%(name)s> set to [%(value)s]::>>>$
	$<<<if_not_empty::$<<data_snippet::autograph-$<current_provider_lastnames::::>$_$<current_provider_firstnames::::>$//\includegraphics[width=30mm]{%s}//image/png//.png::250>>$#3#%s\\#3#% no signature file available::>>>$
	$<<<ph_cfg::argumentsdivider//DEFAULT//%% <%(name)s> set to [%(value)s]::>>>$
	\rule{\widthof{\tiny{}(Der Unterzeichner haftet nicht für unsignierte Änderungen des Inhalts.)}}{.1pt}\\
	$<<ph_cfg::argumentsdivider//#2#//%% <%(name)s> set to [%(value)s]::>>$
	$<<if_not_empty::$<current_provider_name::%(title)s::>$#2#$<current_provider_name::%(title)s %(firstnames)s %(lastnames)s::>$\\#2#$<current_provider_name::%(firstnames)s %(lastnames)s::>$\\::>>$
	$<<if_not_empty::$<current_provider_external_id::Fachgebiet//Ärztekammer::50>$#2#FA für %s\\#2#% no speciality available::>>$
	$<<ph_cfg::argumentsdivider//DEFAULT//%% <%(name)s> set to [%(value)s]::>>$
...
\opening{%
	$<<free_text::Anrede, z.B. "Sehr geehrte/geehrter $<receiver_name::::>$,"//Sehr geehrte Kollegin $<receiver_name::::>$,::140>>$%
}
...
\closing{%
	$<<free_text::Grußformel, z.B. "Mit kollegialen Grüßen,"//Mit kollegialen Grüßen,::140>>$%
}
