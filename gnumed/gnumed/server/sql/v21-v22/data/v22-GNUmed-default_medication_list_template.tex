...
\usepackage{geometry}						% setup margins, use KomaScript tools
...
\lohead{{%
	\parbox[t]{\textwidth}{
		\upshape
		{\centering {\Large Current Medication List}\\}
		{\centering $<today::%d.%B %Y::>$ -- Page \thepage{} of \pageref{LastPage}\\}
		\begin{tabular}[t]{ll}
		...
		\begin{tabular}[t]{ll}
			...
			$<<if_not_empty::$<current_provider_external_id::Fachgebiet//Ärztekammer::>$//{\footnotesize Specialty & %s}\\//% speciality not available::>>$
...
\lehead{{%
	\parbox[t]{\textwidth}{
		...
		\begin{tabular}[t]{ll}
			Doctor & $<current_provider::::>$\\
			$<<if_not_empty::$<current_provider_external_id::Fachgebiet//Ärztekammer::>$//{\footnotesize Speciality & %s}\\//% speciality not available::>>$
			$<<if_not_empty::$<current_provider_external_id::KV-LANR//KV::>$//{\footnotesize LANR & %s}\\//% no LANR available::>>$
		\end{tabular}
	}
}}
