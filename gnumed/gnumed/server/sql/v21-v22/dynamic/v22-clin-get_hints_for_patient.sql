...
BEGIN
	...
	FOR _hint IN SELECT * FROM ref.v_auto_hints WHERE is_active LOOP
		...
		-- re-run against clin._view_emr_journal_without_suppressed_hints which
		-- does not contain suppressed hints -- because the hint may have applied
		-- merely because clin.v_emr_journal includes formatted suppressed hints
		IF _hint_currently_applies THEN
			_sanitized_query := replace(_query, ''clin.v_emr_journal'', ''clin._view_emr_journal_without_suppressed_hints'');
...
-- --------------------------------------------------------------
select gm.log_script_insertion('v22-clin-get_hints_for_patient.sql', '22.0');
