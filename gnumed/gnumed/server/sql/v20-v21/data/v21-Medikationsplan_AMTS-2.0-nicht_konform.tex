...
\lohead{
	...
	\begin{tabu} to \textwidth {>{\raggedleft\arraybackslash}p{7cm}@{\hspace{1.5mm}}X[-1,L]@{\hspace{1cm}}X[-1,R]p{4.3cm}}
		...
		&
			...
			$<praxis_address::\href{http://nominatim.openstreetmap.org/search/$<<praxis_address::%(country)s/%(urb)s/%(street)s/%(number)s::>>$?limit=3}{\mbox{$<<praxis_address::%(street)s %(number)s %(subunit)s,%(postcode)s %(urb)s::55>>$}}::>$
