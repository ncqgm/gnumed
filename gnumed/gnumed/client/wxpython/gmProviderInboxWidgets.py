...
# $Id: gmProviderInboxWidgets.py,v 1.3 2006-05-12 12:21:58 ncq Exp $
...
Leaving message in inbox.""") % handler_key,
		...
		if not handle_item(pk_context = msg[6]):
			...
			return False
		print "seems successfully handled"
		return True
	#--------------------------------------------------------
	...
	def _goto_doc_review(self, pk_context=None):
		pat = gmPerson.set_active_patient(patient=gmPerson.cIdentity(aPK_obj=pk_context))
		gmPlugin.raise_notebook_plugin('gmShowMedDocs')
