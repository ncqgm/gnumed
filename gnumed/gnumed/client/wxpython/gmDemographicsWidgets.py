...
from Gnumed.business import gmAppointments
...
class cKOrganizerSchedulePnl(gmDataMiningWidgets.cPatientListingPnl):
	...
	def reload_appointments(self):
		self._LCTRL_items.set_string_items()		# empty out list
		csv_fname = gmAppointments.get_appointments_for_today_from_korganizer(verbose = _cfg.get(option = 'debug'))
		_log.debug('KOrganizer transfer file: %s', csv_fname)
		if csv_fname is None:
			self._LCTRL_items.set_string_items(items = [['','','','','',_('cannot load appointments')]])
			_log.error('cannot retrieve appointments for today from KOrganizer')
			return
