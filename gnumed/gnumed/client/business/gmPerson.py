...
# $Id: gmPerson.py,v 1.129.2.1 2007-10-18 09:29:47 ncq Exp $
__version__ = "$Revision: 1.129.2.1 $"
		...
		if len(parts_list) == 1:
			...
			if len(sub_parts_list) == 3:
				...
				if date_count == 1:
					...
					queries.append ({
						'cmd': u"SELECT DISTINCT ON (id_identity) vbp.*, %s::text as match_type from dem.v_basic_person vbp, dem.names n WHERE vbp.pk_identity = n.id_identity and lower(firstnames) ~* lower(%s) AND lower(n.lastnames) ~* lower(%s) AND dem.date_trunc_utc('day'::text, dob) = dem.date_trunc_utc('day'::text, %s::timestamp with time zone)",
...
#============================================================
# $Log: gmPerson.py,v $
# Revision 1.129.2.1  2007-10-18 09:29:47  ncq
# - add missing cast ::text in dem.date_trunc_utc() calls
#
