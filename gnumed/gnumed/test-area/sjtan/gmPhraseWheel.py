...
class cMatchProvider_SQL(cMatchProvider):
	...
	field.
	"""
	def __init__(self, source_defs, poolManager = gmPG):
		if poolManager <> None:
			self.dbpool = poolManager.ConnectionPool()

		# sanity check table connections
		self.srcs = []
		for src_def in source_defs:
			src_def['service'] = src_def.get('service', None)
			tmp = {
			...
			if src_def['service'] == None and src_def.has_key('dbapi') and src_def.has_key('connectString'):
				conn = src_def['dbapi'].connect(src_def['connectString'])
			elif src_def.has_key('connectionProvider'):
				conn = src_def['connectionProvider']()
			else:
				conn = self.dbpool.GetConnection(tmp['service'])
				
			if conn is None:
				for src in self.srcs:
					self.releaseConnection(src['conn'])
				raise gmExceptions.ConstructorError, 'cannot connect to service [%s]' % tmp['service']
#			conn.conn.toggleShowQuery
			curs = conn.cursor()
			
			if src_def['service'] <> None:
	...
	def __find_matches(self, search_condition, aFragment):
		matches = []
		for src in self.srcs:
			if src.has_key('connectionProvider'):
				curs = src['connectionProvider']().cursor()
				print "got cursor", curs
			else:	
				curs = src['conn'].cursor()
			
			# THIS IS ADAPTER for editArea
				
			# FIXME: deal with gmpw_score...
			cmd = "%s %s %%s" % (src['query'], search_condition)
			cmd_list = [cmd]

			if src.has_key('extraCondition'):
				cmd_list.append(cmd + ' and ' + src['extraCondition'](*src['extraConditionContext']) )
			cmd_list.reverse()
			for cmd in cmd_list:	
				cmd = '%s limit %s' % (cmd,src['limit'])
				print "command ", cmd  % aFragment
				...
				for row in matching_rows:
					new_matches.append({'data': row[0], 'label': row[1]})
				new_matches.sort(self.__cmp_items)
...
class cPhraseWheel (wxTextCtrl):
	...
	def __init__ (
		...
		**kwargs):
		...
		if not aMatchProvider == None and not isinstance(aMatchProvider, cMatchProvider):
		...
		self.userInputChanging = 0

	def setMatchProvider(self, matcher):
...
#		print "relevant input:", self.relevant_input
	...
	def Append( item, data):
		self._pickList.Append( item, data)
...
#			self._picklist.ProcessEvent (key)
	...
	def _on_timer_fired (self):
		...
		# if true starting of matches then don't show
		if len(self.__currMatches) > 0 and self.userInputChanging :
			# show it
			self.on_resize (None)
			self.__show_picklist()
		else:
	...
	def setUserInputChanging(self, val):
		self.userInputChanging = val
