...
class PrescriptionPanel (wxPanel):
	def __init__(self,parent, id):
		...
		self.UpdateScriptWithList(items)

		self.createMenu() # make menu persistent, so can 
				  # disable unreasonable wishes.
	...
	def UpdateScriptWithList(self, items, clearFirst = 0):
		print "UpdateScriptGoItems", items
		if clearFirst:
			self.list_script.DeleteAllItems()
		for x in range(len(items)):
			key, data = items[x]
			print "line =",x, data[0], data[1], data[2], data[3]
	...
	def createMenu(self):
		...
		aMenu.Append(ID_Undo, _("Undo last action"))
	...
	def OnRightClickUp(self, event):
		...
		# show the menu
		self.PopupMenu(self.aMenu, event.GetPosition())
	...
	def OnExitMenu(self, event):
		print "OnExitMenu" , event.GetString()
#--------------------------------------------------------------------
