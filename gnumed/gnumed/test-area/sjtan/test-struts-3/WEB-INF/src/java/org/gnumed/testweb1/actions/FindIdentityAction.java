...
public class FindIdentityAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			DemographicDataAccess access = (DemographicDataAccess) servlet
					.getServletContext().getAttribute(
			...
			request.getSession().setAttribute(
					Constants.Session.DEMOGRAPHIC_DETAILS, list);
			...
			errors.add(getClass().toString(), new ActionMessage(
					"errors.writing.response", e));

		} catch (IllegalAccessException e) {
			log.error(e + e.getLocalizedMessage());
			errors.add(getClass().toString(), new ActionMessage(
					"errors.copy.object", form, detail));
			...
			errors.add(getClass().toString(), new ActionMessage(
					"errors.find.demographics", e.getCause()));
