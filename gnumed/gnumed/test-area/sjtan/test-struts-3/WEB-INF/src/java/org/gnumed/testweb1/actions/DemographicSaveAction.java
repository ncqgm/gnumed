...
public class DemographicSaveAction extends Action {
    ...
    HttpServletResponse response) {
        ...
        try {
            ...
            //          BeanUtils.populate(form, map); // THIS WON"T WORK, DynaForm property to accessible with populate
            //
            //          log.info("After populate: the map size=" + Integer.toString(map.size()) );
            ...
            String username = (String) PropertyUtils.getSimpleProperty(form, "givenname");
            ...
            //LoginModule module = (LoginModule) request.getSession().getAttribute(Constants.LOGIN_MODULE);
...
//                log.info( detail + " has attribute '" + me.getKey() + "' : " + me.getValue());
//            }
        ...
        //    org.gnumed.testweb1.global.Util.logBean(log, form);
         ...
         //   org.gnumed.testweb1.global.Util.logBean(log, detail);
            ...
            messages.add("error in form", new ActionMessage("errors.detailSave" , e , e.getCause()));
    ...
    /** after saving the modified or new demographicDetail, update it
     * if it is stored in the session attribute DEMOGRAPHIC_DETAILS which
     *is a list of found demographic details.
     */
    void updateSessionDemographicList(DemographicDetail detail, HttpSession session) {
        if (session.getAttribute(Constants.Session.DEMOGRAPHIC_DETAILS) != null) {
            java.util.List l = java.util.Collections.
            synchronizedList((java.util.List) session.getAttribute(
