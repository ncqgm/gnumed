...
from gmEditAreaTemplate import *
TBx = 1		#TextBox
CHBx = 2	#CheckBox	
RBn = 3		#RadioButton
CMBx =4		#ComboBox
...
def getTopMostWindow( w ):
	while w.GetParent() <> None:
		w = w.GetParent()
	return w	
...
class EditAreaFactoryClientPanel:

	def __init__(self):
		self.factory = EditAreaFactory()
	
	def layoutLines(self):
		self.factory.getLines()
...
class EditArea2(wxPanel):
	def __init__(self, parent, id):
		...
		self.factory = EditAreaFactory()
		self.factory.setParentContainer(self)
		

	def add(self, propName,  type=TBx ,weight=2,  displayName = None,  newline = 0, constrained = 1):
		self.factory.add(propName,  type ,weight,  displayName,  newline , constrained )
	
	def map( self, name1, name2, order =0, defaultValueFunction = None):
		print "mapping with factory", name1, name2
		self.factory.map(name1, name2, order, defaultValueFunction)

	def browse(self, list):
		self.factory.browse(list)
	
	
	def ddl( self, name, statement):
		self.factory.ddl( name, statement)

	def ext_ref(self, entity, ext):
		self.factory.ext_ref(entity, ext)

	def setExtRef(self, ref):
		self.factory.setExtRef(ref)
	
	def setExtRefId(self, id):	
		self.factory.setExtRefId(id)
		
	def target(self, target):
		self.factory.target(target)

	def ref(self, fromT, toT):
		self.factory.ref(fromT, toT)
		
	def finish(self):
		self.factory.initWidgetLines()
		self.factory.makeWidgetLines(self)
		self.factory.finish()
		self.layoutLines()
	
	def updateList(self):
		self.factory.updateList()


	def layoutLines(self):
		lines = self.factory.getLines() 
		listCtrl = self.factory.getListCtrl()
		for line in lines:
		...
		self.topSizer.Add(listCtrl, wxEXPAND)	
		self.SetSizer(self.topSizer)
		self.SetAutoLayout(true)
		self.topSizer.Fit(self)
		self.topSizer.SetSizeHints(self)
		self.topSizer.SetItemMinSize( listCtrl, 1000, 600)
		self.Layout()

		
class EditAreaTraditional(wxPanel):

	def __init__(self, parent, id):
		wxPanel.__init__(self, parent, id, wxDefaultPosition, wxDefaultSize, style = wxNO_BORDER | wxTAB_TRAVERSAL)
		#self.topSizer. AddGrowableCol(0)
		self.factory = EditAreaFactory(defaultLabelWeight = 1 )
		self.layoutManager = gmEditAreaLayoutManager()
		self.layoutManager.setFactory(self.factory)
		self.factory.setParentContainer(self)
	...
	def finish(self):
		self.factory.initWidgetLines()
		self.editAreaSizer = self.layoutManager.layout(self, self.factory.getWidgetConfigInLines())
		self.factory.finish()
		self.layoutLines()
	...
	def layoutLines(self):
#		layoutManager = gmEditAreaLayout()
#		for line in lines:
#			lineSizer = wxBoxSizer(wxHORIZONTAL)
#			for (widget, weight, style) in line:
#				lineSizer.Add( widget, weight, style)
#			self.topSizer.Add( lineSizer, 2, wxEXPAND)
		listCtrl = 	self.factory.getListCtrl()
		
		#self.topSizer = wxFlexGridSizer(3,1,10,10)
		#self.topSizer = wxGridSizer(3,1,10,10)

		
		self.topSizer = wxBoxSizer(wxVERTICAL)
		self.topSizer.Add(self.editAreaSizer, 1, 0)
		#self.removeGrowableRow(0)
		#self.topSizer.SetItemMinSize( self.editAreaSizer, 500, 500)
		self.topSizer.Add(self.factory.getSaverSizer(),0, 0)
		self.topSizer.Add(listCtrl,10,  wxEXPAND | wxGROW)	
		#self.topSizer.AddGrowableRow(2)
		#self.topSizer.AddGrowableCol(0)
...
class EditAreaFactory:
	...
	def __init__(self, defaultLabelWeight = 1):
	...
	def makeWidgetLines(self, parent):
	   lines = self.getWidgetConfigInLines()
	   for l in lines:
		   for definition in l:
		   	self.createWidget(parent, definition)
	   self.lparent = parent

	def getWidgetConfigInLines(self):
		lines = []
		line = []
		for 	( propName, type, weight,displayName, newline, constrained) in  self.widgetConfig:
			if type in [ TBx, GMDI, CMBx, PWh] :
				weight += 2
				if displayName == None:
					displayName = propName
				line.append( (str(wxNewId()), LAB , self.defaultLabelWeight, displayName, 0, 0) )
			
			line.append( (propName, type, weight ,displayName, newline, constrained ) )
			if newline  == 1:
				lines.append(line)
				line = []
		return lines		
		...
		if type == PWh:
			...
			#print "making widget PHRASEWHEEL"
			widget = gmPhraseWheel.cPhraseWheel( parent =parent, id = -1, size = wxDefaultSize, pos = wxDefaultPosition , id_callback = selector.id_callback)
			widget.setMatchProvider( self.phraseWheelMatcher[propName] )
		
		if constrained == 0 and (type == PWh or type == CMBx):
			EVT_KILL_FOCUS( widget, self.checkForNewIdEvent)
	...
	def finish(self):
		...
		self.makeListControl(self.parent)
	#	self.topSizer.Add(self.lineSizer, wxEXPAND)
		
	
		saverSizer = SaverSizer(self.parent, -1) 
		saverSizer.setOkCommand( self.saveOrUpdate)
		saverSizer.setCancelCommand( self.clear)
		saverSizer.setDeleteCommand(self. delete)

		self.saverSizer = saverSizer
		
		self.lines.append( [( saverSizer, 1, wxEXPAND)] )
		self.lines.append( [(self.listCtrl, 10, wxEXPAND)] )
		#self.topSizer.AddGrowableRow( self.rowCount)
	...
	def getSaverSizer(self):
		return self.saverSizer
	...
	def map( self, widgetName, sqlNames, order , defaultValueFunction = None):
		if order == 1:
	...
	def _setPhraseWheelMatcher(self, widgetName, sqlNames):
		...
		self.phraseWheelMatcher[widgetName] = gmPhraseWheel.cMatchProvider_SQL( source_defs, poolManager = None) 
		print "self.phraseWheelMatcher = ", self.phraseWheelMatcher	
	...
	def updateParentAndChildSelections( self, widgetName):
		if self.constrained[widgetName] in [0, 2]:
			self.checkForNewId(widgetName)
			return
	...
	def checkForNewIdEvent(self, event):
		"""the purpose of this kill focus event handler is to check for non-constrained fields
		having their value changed, which is by default creating a new referenced entity.
		This will orphan the old referenced entity, but it will be available for selection 
		and re-usage. The con is that many mispelt useless entities will end up orphaned and not very
		useful.
		"""
		print "kill focus from ", event.GetEventObject().GetName()
		self.checkForNewId( event.GetEventObject().GetName() )
		event.Skip()

	def checkForNewId(self, widgetName):
		text = self.getWidgetByName(widgetName).GetValue()
		table = self.getTableFromWidget(widgetName)	
		consistentChoices = self._selectList(widgetName, text)
		print "check for new id ,", consistentChoices , "setting new id to None  (y = 1, n = 0) :", consistentChoices ==[]
		if consistentChoices == []:
			self.selectedIndex[table] = None
...
#				condition.append('%s.id_%s = %s.id' % ( t, self.extRef, self.extRef ) )
	...
	def parseTargetFields(self,target, recursionCount,  visited):
		...
		for r in refList:
			...
			if not  self.selectedIndex.has_key(r) or self.selectedIndex[r] == 0 or self.selectedIndex[r] ==None: # cascade saveNewTarget to unsaved row in referenced table
				if (r in visited):
...
class MedicationEditArea(EditAreaTraditional):
	
	def __init__(self, parent, id):
		EditAreaTraditional.__init__(self, parent, id)
...
#		self.ddl("atc", "create view atc_view as select  lda.id_drug as id, atc.code, atc.text as name from atc , link_drug_atc lda where lda.atccode=atc.code")
		...
		self.map("date", "prescription.date", defaultValueFunction = now )
...
class PastHistoryEditArea(EditAreaTraditional):
	def __init__(self, parent, id):
		EditAreaTraditional.__init__(self, parent, id)
		#self.add("coding system", CMBx, newline = 1)
		self.add("condition", PWh, weight = 4,  newline  = 1 )
		self.add("laterality", LAB)
		self.add("left",  RBn)
		self.add("right", RBn)
		self.add("both",  RBn, newline = 1)
		self.add("notes",  weight = 4, newline = 1)
		self.add("age onset" )
		...
		self.browse([ ( "past_history.year_onset" , 60), ("past_history.age_onset", 60), ("condition.description", 200) , ("past_history.aleft", 40, 'left'), ( "past_history.aright", 40, 'right') , ("past_history.aboth", 40, 'both') , ("past_history.active", 20), ("past_history.notes", 200 ) ] )
...
class DemographicEditArea(EditAreaTraditional):
	def __init__(self, parent, id):
		EditAreaTraditional.__init__(self, parent, id)
