...
# Created:      2002/11/20
# Version:      0.1
# RCS-ID:       $Id: multisash.py,v 1.3 2005-02-16 07:33:51 cfmoro Exp $
...
# $Id: multisash.py,v 1.3 2005-02-16 07:33:51 cfmoro Exp $
...
class wxMultiSash(wxWindow):
    ...
    #---------------------------------------------        
    def __find_bottom_leaf(self, split):
        """
        Recursively find the bottom leaf.
        """
        if isinstance(split.view1,wxMultiSplit):
            print "parent split (view1): %s [%s]" % (split.view1.__class__.__name__, id(split.view1))
            return self.__find_bottom_leaf(split.view1)
        elif isinstance(split.view2,wxMultiSplit):
            print "parent split (view2): %s [%s]" % (split.view2.__class__.__name__, id(split.view2))
            return self.__find_bottom_leaf(split.view2)
        else:
            print "found bottom split: %s [%s]" % (split.__class__.__name__, id(split))
            if not split.view2 is None:
                print "bottom leaf (view2): %s [%s]" % (split.view2.__class__.__name__, id(split.view2))
                return split.view2
            else:
                print "bottom leaf (view1): %s [%s]" % (split.view1.__class__.__name__, id(split.view1))
                return split.view1
