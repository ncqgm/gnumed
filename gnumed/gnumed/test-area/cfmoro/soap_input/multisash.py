...
# Created:		2002/11/20
# Version:		0.1
# RCS-ID:		$Id: multisash.py,v 1.2 2005-02-15 13:12:06 ncq Exp $
...
# $Id: multisash.py,v 1.2 2005-02-15 13:12:06 ncq Exp $
...
class wxMultiSash(wxWindow):
	...
	def get_focussed_leaf(self):
		...
		Typically used to process some action over the focused widget.
		"""
...
class wxMultiSplit(wxWindow):
	...
	def DestroyLeaf(self,caller):
		...
		print "leaf 1: %s [%s]" % (self.view1.__class__.__name__, id(self.view1))
		print "leaf 2: %s [%s]" % (self.view2.__class__.__name__, id(self.view2))
		if not self.view2:				# We will only have 2 windows if we need to destroy any
			print "there is no leaf 2, cannot destroy any leaf"
			return
		parent = self.GetParent()		# Another splitview
		print "parent   : %s [%s]" % (parent.__class__.__name__, id(parent))
		print "multiview: %s [%s]" % (self.multiView.__class__.__name__, id(self.multiView))
		if parent == self.multiView:	# We'r at the root
			...
			if caller == self.view1:
				...
				self.view2.Destroy()
				self.view2 = None
			# Gnumed: sets the only existing leaf (view1) as bottom
			self.view1.set_bottom()	  
...
class wxMultiViewLeaf(wxWindow):
	...
	def set_bottom(self):
		"""
		Set current leaf as the bottom leaf. Typically, new widgets
		will be added under bottom leaf.
		"""
		self.multiView.bottom_leaf = self
		print "bottom soap editor leaf:", self.__class__.__name__, id(self)
		self.set_focus()
...
#----------------------------------------------------------------------
# $Log: multisash.py,v $
# Revision 1.2  2005-02-15 13:12:06  ncq
# - added lots of prints and comments
#
#
