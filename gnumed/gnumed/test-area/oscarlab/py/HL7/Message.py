...
"""
from  HL7.V2_3.MSH import MSH
from  HL7.V2_3.PID import PID 
lineBreak = "\n"
class Message:
	def __init__(self, now):
		self.now = now
		self.msh, self.pid, self.current = None, None, None

	def accept(self, visitor):
		if self.msh:
			self.msh.accept(visitor)
		if self.pid:
			self.pid.accept(visitor)

	
	def Parse(self, data):
		global lineBreak
		lines = data.split(lineBreak)
		for l in lines:
			prefix = l[0:3]
			
			if prefix == "MSH":
				self.msh = MSH()
				self.current = self.msh.Parse(l)
				
			elif prefix == "PID":
				self.pid = PID()
				self.current  = self.pid.Parse(l)

			elif prefix == "NTE":
				if self.current:
					self.current.Parse(l)
