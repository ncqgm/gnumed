...
read -p "press enter to start gnumed database install:" 
echo 
echo
...
echo createdb -h.127.0.0.1 -Upostgres -O "gm-dbo" gnumed

createdb -h127.0.0.1 -Upostgres --owner "gm-dbo" gnumed

echo "creating the user gm-dbo ( may not work, if already exists for gnumed_v2, as gm-dbo is a global object, and can't be dropped)"
echo
...
createlang  -h127.0.0.1 -Upostgres "plpgsql" gnumed  ||  echo "failed to createlang plpgsql with user postgres" exit;
